...
and `unread` says so. The file may have been parsed with nothing taken from the name — a form the
parser does not record, or a comment — and then `symbol_found_on_disk` is `true` beside empty
lists. The scan may have been cut short before it began. In both cases `narrowed_by` says by
