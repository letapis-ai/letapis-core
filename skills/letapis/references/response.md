...
|---|---|
| `symbol_found_on_disk` | **reads as a claim about the disk; is a claim about the files this lookup handed to a reader.** `true` means such a file carries the name — as a definition, a call, a read, a string, or only as text its reader took nothing from: a form the reader does not record, a name written in a comment, a file the reader gave up on. So `true` beside every list empty is a real answer, and `narrowed_by` names why nothing was listed. `false` speaks only for files handed to a reader: a name written in a language with no extractor returns `false` while sitting in those files many times over — `unread` and `narrowed_by` below are what tell you that is what happened |
| `hint` | the one that carries the real reason — an extension with no extractor, a scan narrowed before it began, or a file its reader took nothing from. It opens with `NOT FULLY SCANNED` when the zero speaks for part of what is on disk rather than all of it. Without it the flag above misleads |
...
| `unparsed` | files that were opened and whose reader gave up, counted per extension. Different from `unread`: there the language is unknown, here the file is. Such a file that carries the name is also counted under `nothing_taken` in `narrowed_by`, and `symbol_found_on_disk` is `true` |
...
| `narrowed_by` | what kept the answer from listing everything: `{by, detail, carried_the_name}`. Some cuts come **before** the scan — a `folder` you named and the watched folders left outside it, a group, a hidden folder, the watch rules of the folder itself, an extension with no reader (`no_extractor`). Two come **after** a file was handed to a reader — a name treated as background noise (`noise_name`), and a name standing in the file with nothing taken from it (`nothing_taken`), a file the reader gave up on included. `carried_the_name` is a number when those files were opened and carry your name, and `null` when they were never opened — such a row says nothing about the name. A number does not by itself decide `symbol_found_on_disk`: beside `noise_name` or `nothing_taken` the files were handed to a reader and the field is `true`; beside `no_extractor` they were matched as text and never parsed, and the field answers from the other files alone |
